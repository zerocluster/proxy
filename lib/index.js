...
export default class {
    ...
    async run () {
        ...
        for ( const name in this.#options.zones ) {
            ...
            if ( zone.proxy ) zone.proxy = Array.isArray( zone.proxy ) ? Proxy.new( ...zone.proxy ) : Proxy.new( zone.proxy );
    ...
    async #getProxy ( connection ) {
        if ( connection.options.session ) connection.options.session = connection.options.username + "/" + connection.options.zone + "/" + connection.options.session;
