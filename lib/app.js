...
export default class extends App {
    ...
    async #run () {
        ...
        for ( let port = minPort; port <= maxPort; port++ ) {
            ...
            await proxyServer.ref().listen( { "address": config.listen, port } );
    ...
    async #auth ( connection, password ) {
        ...
        if ( user.whitelist && !subnets.get( user.whitelist ).includes( connection.remoteAddress ) && !subnets.get( "local" ).includes( connection.remoteAddress ) && !subnets.get( "private" ).includes( connection.remoteAddress ) ) return false;
