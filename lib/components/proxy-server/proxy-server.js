...
export default class {
    ...
    async #authorize ( connection, password ) {
        ...
        if ( user.whitelist && !subnets.get( user.whitelist ).hasRangesIntersecting( connection.remoteAddress ) ) return false;
    ...
    #checkConnection ( address, port ) {
        if ( this.#denySubnets.size ) {
            ...
            for ( const subnet of this.#denySubnets ) {
                if ( subnets.get( subnet )?.hasRangesIntersecting( address ) ) return;
            }
        }
