...
export default class {
    ...
    async start () {
        ...
        if ( this.config.nginx.enabled && this.app.nginxUpstream ) {
            await this.app.nginxUpstream.addProxy( "proxy-server", {
    ...
    async #authorize ( connection, password ) {
        ...
        if ( user.whitelist && !subnets.get( user.whitelist ).findIntersectingRanges( connection.remoteAddress ).length ) return false;
    ...
    #checkConnection ( address, port ) {
        if ( this.#denySubnets.size ) {
            ...
            for ( const subnet of this.#denySubnets ) {
                if ( subnets.get( subnet )?.findIntersectingRanges( address ).length ) return;
            }
        }
